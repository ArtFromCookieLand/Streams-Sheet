...
**[ADDING-SONGS.md](ADDING-SONGS.md)** is the playbook for the spreadsheet itself: what to do,
and in which order, when a single, an album, a new category or a song that is already on Spotify
has to be added. It is written to be pasted into the sheet.
...
* 2.0.0 (22/9/26)
    ...
    * **New spreadsheet layout**, the same in Latest, Tools and every archive: the discography total on row 2, the solo total on row 3, albums and categories on 4-27, spare rows to 48, the songs' header on 49 and songs from row 50 with no upper limit. Album totals in Latest and the archives are generated from the Tracklist, so a new song always counts. A one-off script migrated both spreadsheets.
    * **Adding songs is one menu item.** Pending → **Add Pending Songs** puts each song at the end of its album's block, at the same row in every sheet, and renumbers the Tracklist. The batch is checked first; if a row has a problem nothing changes and the reason appears next to it.
    * **Songs can be added before release** with the **upcoming** status: no track ID needed, held at 0, and never blocking the update. When it appears on Spotify the import links it and says so.
    * **New tracks are found after every import.** Another edition of a tracked recording is ignored automatically, an announced song is linked to its ID, and anything else lands in Pending with its album and cover key suggested.
    * **Covers fill themselves in** for new cover keys, from the scraped album's artwork or Spotify's embed lookup.
    * **New albums and categories**: a name and type in Categories, then **Add New Categories** opens its row everywhere, in the right place for its type, without moving any song.
    * **A Checks menu with eleven read-only health checks**: row alignment across every sheet, album totals against their songs, archive dates, the import, covers, merged cells, spare rows, categories and sources. The quick ones run after every update and only speak up if something is wrong.
* 1.1.4 (21/9/26)
